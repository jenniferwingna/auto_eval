...
SETUP:
    1. Copy this file to chat_server.py:
       cp chat_server_example.py chat_server.py

    2. Set your DeepSeek token (choose one):
       a) export DEEPSEEK_TOKEN=sk-your-token-here
       b) Create .env file with: DEEPSEEK_TOKEN=sk-your-token-here

    3. Run: python3 chat_server.py
       → http://localhost:8765
...
每条用例请严格按照以下 JSON 格式输出（只输出 JSON 数组，不要其他内容）：
[
  {{
...
要求：query 自然中文口语（含口语噪音更好），覆盖不同车控子域，包含正例和负例，断言可自动验证（不要 rubric）。"""

        print(f"🔧 Generate: {count} cases, topic='{topic}'")
        result = self._call_deepseek([{"role": "user", "content": prompt}], max_tokens=4000)
        ...
        try:
            ...
            cases = [{"error": "Failed to parse", "raw": result[:300]}]
